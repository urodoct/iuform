...
#' @param Checked status of the check box at the start
#' @param section the section where the element will be placed
#'
#'
#' @export
new_checkbox <- function(label,name, must_fill=F, section, Checked=F){
  ...
  blanky <- glue::glue("{if(Checked)return ('T') else return ('F')}")
  blanky2 <-glue::glue(
    "labelMandatory('",
    "{element_name}",
    "')"
  ...
  part1 <- glue::glue("  checkboxInput('{label}','{element_name}', {if(must_fill)return ('{blanky2}')else return ('')}, {blanky}),",)
