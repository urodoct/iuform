...
#' @param ticks status of slider ticks
#' @param section the section where the element will be placed
#'
#'
#' @export
new_slider <- function(label,name, must_fill=F,ticks=F ,section){
  ...
  blanky <- glue::glue("{if(ticks)return ('T') else return ('F')}")
  blanky1 <-  glue::glue("!{label}")
  blanky2 <-glue::glue(
    "labelMandatory('",
    "{element_name}",
    "'),"
  ...
  part1 <- glue::glue("  sliderInput('{label}','{element_name}', {if(must_fill)return ('{blanky2}')else return ('')}{blanky1},ticks={blanky}),",)
